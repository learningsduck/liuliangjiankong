...
- **GB 进制**：在 **新增/编辑** 对话框里为 **每台服务器** 选择 **「GB: 1024³」** 或 **「GB: 1000³」**，写入 `servers.yaml` 该项的 **`gb_base`**（`1024` 或 `1000`）；影响列表「已用/套餐」及 SSH 下 **「月流量上限 / 面板已用」** 的 GB 与字节换算。未写时默认 **1024**。
...
### 用「面板已用」与 vnstat 对齐（**仅 SSH**；搬瓦工以 API 为准，编辑里不提供面板已用）

1. 在主页点 **「刷新」**，确保该行已成功拉取流量。  
2. **编辑** 该 **ssh_vnstat** 服务器，在 **「面板已用(GB)」** 填入商家面板上的 **本月已用** 数字，点 **保存**。  
...
**搬瓦工**：主页已用直接跟 KiwiVM API；保存搬瓦工配置时会自动去掉 **`panel_anchor_*` / `used_offset_bytes`**（若旧配置里曾写过）。
