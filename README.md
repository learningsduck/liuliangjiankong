...
- **表头列顺序**：先点击某一列表头选中列，再用顶部 **「列上移 / 列下移」** 或表格右侧 **「上移 / 下移」** 调整显示顺序；会写入与程序同目录的 **`column_order.json`**（可删之恢复默认）。若界面没有这些按钮，请 **`git pull`** 后重新运行 **`python app_gui.py`**，或重新执行 **`build_exe.bat`** 生成新版 exe。
